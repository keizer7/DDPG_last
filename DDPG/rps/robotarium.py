...
class Robotarium(RobotariumABC):
    ...
    def step(self):
        ...
        # Update dynamics of agents---三个追捕者
        ...
        if (self.show_figure):
            ...
            for i in range(self.number_of_robots):
                ...
                self.chassis_patches[i].xy = self.poses[:2, i] + self.robot_length / 2 * np.array(
                    (np.cos(self.poses[2, i] + math.pi / 2), np.sin(self.poses[2, i] + math.pi / 2))) + \
                                             0.04 * np.array((-np.sin(self.poses[2, i] + math.pi / 2), np.cos(
                    self.poses[2, i] + math.pi / 2))) + self.robot_length / 2 * np.array(
                ...
                self.right_wheel_patches[i].center = self.poses[:2, i] + self.robot_length / 2 * np.array(
                    (np.cos(self.poses[2, i] + math.pi / 2), np.sin(self.poses[2, i] + math.pi / 2))) + \
                                                     0.04 * np.array((-np.sin(self.poses[2, i] + math.pi / 2), np.cos(
                    self.poses[2, i] + math.pi / 2))) + self.robot_length / 2 * np.array(
                    (np.cos(self.poses[2, i]), np.sin(self.poses[2, i])))
                ...
                self.left_wheel_patches[i].center = self.poses[:2, i] + self.robot_length / 2 * np.array(
                    (np.cos(self.poses[2, i] - math.pi / 2), np.sin(self.poses[2, i] - math.pi / 2))) + \
                                                    0.04 * np.array((-np.sin(self.poses[2, i] + math.pi / 2), np.cos(
                    self.poses[2, i] + math.pi / 2))) + self.robot_length / 2 * np.array(
                    (np.cos(self.poses[2, i]), np.sin(self.poses[2, i])))
                ...
                self.right_led_patches[i].center = self.poses[:2, i] + 0.75 * self.robot_length / 2 * np.array(
                    (np.cos(self.poses[2, i]), np.sin(self.poses[2, i]))) - \
                                                   0.04 * np.array(
                    (-np.sin(self.poses[2, i]), np.cos(self.poses[2, i]))) + self.robot_length / 2 * np.array(
                    (np.cos(self.poses[2, i]), np.sin(self.poses[2, i])))
                self.left_led_patches[i].center = self.poses[:2, i] + 0.75 * self.robot_length / 2 * np.array(
                    (np.cos(self.poses[2, i]), np.sin(self.poses[2, i]))) - \
                                                  0.015 * np.array(
                    (-np.sin(self.poses[2, i]), np.cos(self.poses[2, i]))) + self.robot_length / 2 * np.array(
                    (np.cos(self.poses[2, i]), np.sin(self.poses[2, i])))
                # self.base_patches[i].center = self.poses[:2, i]
                # self.path[i].set_xdata(np.append(self.path[i].get_xdata(), self.poses[0, i]))
                # self.path[i].set_ydata(np.append(self.path[i].get_ydata(), self.poses[1, i]))
                # self.axes.add_line(self.path[i])
                #
                # if len(self.path[i].get_xdata()) > 50:
                #     # Remove the oldest point from the trajectory
                #     self.path[i].set_xdata(self.path[i].get_xdata()[1:])
                #     self.path[i].set_ydata(self.path[i].get_ydata()[1:])

            # 刷新和更新图形界面
            self.figure.canvas.draw_idle()
            self.figure.canvas.flush_events()

    def diff2omni(self,v_diff,pose):
        # v_diff=np.array([[v],[w]])
        vx=v_diff*cos(pose[2])
        vy=v_diff*sin(pose[2])
        v_omni=np.array([[vx],[vy]])
        return v_omni

    def omni2diff(self,v_omni):
        # omni=np.array([[vx],[vy]])
        speed = sqrt(v_omni[0] ** 2 + v_omni[1] ** 2)

        if speed > self.max_linear_velocity:
            speed = self.max_linear_velocity

        w_max=self.max_angular_velocity
        v_radius=atan2(v_omni[1],v_omni[0])
        diff_radians=self.robot_radius-v_radius
        ...
        if diff_radians < 0.05 and diff_radians > -0.05:
            w = 0
        else:
            w = -diff_radians / 0.2
            if w > w_max:
